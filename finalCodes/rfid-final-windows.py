...
def scan_epc(epc):
    ...
    winsound.Beep(1000, 200)  # Beep on scan
...
def export_pdf():
    ...
    elements.append(Paragraph("\U0001F9FE Smart Tray Invoice", styles["Title"]))
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.Markdown("## \U0001F680 Auto‐Scan Smart Tray Demo")
    ...
    bill_tbl = gr.Dataframe(value=get_bill_df(), label="Tray Items", interactive=False)
    summary = gr.Textbox(value=summary_text(), label="Summary", lines=3, interactive=False)
    reset_btn = gr.Button("Reset Tray")

    with gr.Row():
        export_csv_btn = gr.Button("\U0001F4E5 Export CSV")
        export_pdf_btn = gr.Button("\U0001F4C4 Export PDF")
    ...
    export_csv_btn.click(fn=handle_csv_export, outputs=[export_file])
    export_pdf_btn.click(fn=handle_pdf_export, outputs=[export_file])
    ...
    def manual_reset():
        scanned_items.clear()
        return "\U0001F9F9 Tray cleared", get_bill_df(), summary_text()
    ...
    def read_from_rfid_serial():
        try:
            ...
            while True:
                ...
                if raw:
                    print(f"📡 Read tag: {raw}")
                    scan_epc(raw)
        except Exception as e:
            print("❌ Error reading RFID:", e)

    def start_thread():
        threading.Thread(target=read_from_rfid_serial, daemon=True).start()
